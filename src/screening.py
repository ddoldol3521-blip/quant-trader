...
from src.data.kr_universe import get_universe
from src.strategies import STRATEGIES

LOOKBACK_DAYS = 400  # 지표 계산(최대 252일 롤링)에 필요한 과거 데이터 확보용 여유분


def get_multi_market_universe(markets: list, limit: int) -> pd.DataFrame:
    """여러 시장의 시가총액 상위 종목 목록을 합쳐서 가져온다 (중복 종목 제거)."""
    frames = [get_universe(market, limit) for market in markets]
